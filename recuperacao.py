...
Como integrar ao seu app existente (padrão MultiApp / option_menu):
- Salve este arquivo como `recuperacao.py` na mesma pasta do seu
  `Login_necessidade.py` (ou onde os outros módulos de subpágina estão).
- No arquivo principal, já deve existir:
      import recuperacao
      ...
      if subpage == "Recuperação por motivo":
          recuperacao.subpage()
- Este arquivo NÃO chama st.set_page_config() — isso já é feito uma
  única vez no arquivo principal (Login_necessidade.py). Chamar de
  novo aqui quebraria o app com erro de "set_page_config can only be
  called once".
